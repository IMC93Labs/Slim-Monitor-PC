...
public sealed class TaskbarOverlayForm : Form
{
    ...
    private readonly System.Windows.Forms.Timer _shellTimer = new() { Interval = 300 };
    ...
    private static bool IsTaskbarFrontmostAtProbe(IntPtr taskbar, Rectangle rect)
    {
        var point = rect.Width >= rect.Height
            ? new NativeMethods.POINT(rect.Right - 2, rect.Top + Math.Max(1, rect.Height / 2))
            : new NativeMethods.POINT(rect.Left + Math.Max(1, rect.Width / 2), rect.Bottom - 2);
